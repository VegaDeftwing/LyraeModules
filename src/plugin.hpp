...
struct HexKnob : app::SvgKnob {
  HexKnob() {
    ...
    maxAngle = 0.8 * M_PI;
    setSvg(
        APP->window->loadSvg(asset::plugin(pluginInstance, "res/HexKnob.svg")));
  }
};
...
struct SmallHexKnob : app::SvgKnob {
  SmallHexKnob() {
    ...
    setSvg(APP->window->loadSvg(
        asset::plugin(pluginInstance, "res/SmallHexKnob.svg")));
  }
};
...
struct SmallHexKnobInv : app::SvgKnob {
  SmallHexKnobInv() {
    ...
    maxAngle = 0.8 * M_PI;
    setSvg(APP->window->loadSvg(
        asset::plugin(pluginInstance, "res/SmallHexKnobInverted.svg")));
  }
};
...
struct MedHexKnob : app::SvgKnob {
  MedHexKnob() {
    ...
    maxAngle = 0.8 * M_PI;
    setSvg(APP->window->loadSvg(
        asset::plugin(pluginInstance, "res/MedHexKnob.svg")));
  }
};
