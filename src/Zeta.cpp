...
struct Zeta : Module {
  enum ParamIds { FLIPPED_PARAM, ALT_PARAM, NUM_PARAMS };
...
//   Zeta *module;
//   SomeWidgetBuffer(Zeta *m) { module = m; }
//   void step() override {
//     // if(module->dirty){
...
//     Remember to use a FramebufferWidget to avoiding redrawing your SVG
//     every frame.
//     */

//     std::shared_ptr<Svg> svg =
//         APP->window->loadSvg(asset::plugin(pluginInstance, "res/Zeta.svg"));
...
//           svg = APP->window->loadSvg(
//               asset::plugin(pluginInstance, "res/Zetab.svg"));
//         } else {
//           svg = APP->window->loadSvg(
//               asset::plugin(pluginInstance, "res/ZetabAlt.svg"));
//         }

//       } else if (zeta->paramQuantities[Zeta::ALT_PARAM]->getValue() == 2.f) {
//         if (zeta->paramQuantities[Zeta::FLIPPED_PARAM]->getValue() == 1.f) {
//           svg = APP->window->loadSvg(
//               asset::plugin(pluginInstance, "res/Zetac.svg"));
//         } else {
//           svg = APP->window->loadSvg(
//               asset::plugin(pluginInstance, "res/ZetacAlt.svg"));
//         }
//       } else {
//         if (zeta->paramQuantities[Zeta::FLIPPED_PARAM]->getValue() == 1.f) {
//           svg = APP->window->loadSvg(
//               asset::plugin(pluginInstance, "res/Zeta.svg"));
//         } else {
//           svg = APP->window->loadSvg(
//               asset::plugin(pluginInstance, "res/ZetaAlt.svg"));
//         }
//       }
//     }
...
struct ZetaWidget : ModuleWidget {
  ...
  void appendContextMenu(Menu *menu) override {
    ...
    struct ZetaOutputAltItem : MenuItem {
      ...
      void step() override {
        rightText = CHECKMARK(
            zeta->paramQuantities[Zeta::FLIPPED_PARAM]->getValue() == 1.f);
      }
    };
    ...
    struct ZetaOutputAltaItem : MenuItem {
      ...
      void step() override {
        rightText = CHECKMARK(
            zeta->paramQuantities[Zeta::ALT_PARAM]->getValue() == 0.f);
      }
    };
    ...
    struct ZetaOutputAltbItem : MenuItem {
      ...
      void step() override {
        rightText = CHECKMARK(
            zeta->paramQuantities[Zeta::ALT_PARAM]->getValue() == 1.f);
      }
    };
    ...
    struct ZetaOutputAltcItem : MenuItem {
      ...
      void step() override {
        rightText = CHECKMARK(
            zeta->paramQuantities[Zeta::ALT_PARAM]->getValue() == 2.f);
      }
    };

    menu->addChild(new MenuEntry);
    ZetaOutputAltItem *altOutput =
        createMenuItem<ZetaOutputAltItem>("Flip Panel");
    altOutput->zeta = zeta;
    menu->addChild(altOutput);
    ZetaOutputAltaItem *altaOutput =
        createMenuItem<ZetaOutputAltaItem>("Green Echos");
    altaOutput->zeta = zeta;
    menu->addChild(altaOutput);
    ZetaOutputAltbItem *altbOutput =
        createMenuItem<ZetaOutputAltbItem>("Blue Fish");
    altbOutput->zeta = zeta;
    menu->addChild(altbOutput);
    ZetaOutputAltcItem *altcOutput =
        createMenuItem<ZetaOutputAltcItem>("Purple Demons");
